// fix-appliquer-segou-complet.js
...
//   node fix-appliquer-segou-complet.js

const fs = require('fs');
const NOM_FICHIER = 'region-san.html';
...
tenter('selHeure modifie', function () {
  var ancien = "function selHeure(h,id){mHeure=h;";
  var nouveau = "function selHeure(h,id,nbPlaces){mHeure=h;if(nbPlaces) placesHoraireChoisi=parseInt(nbPlaces,10)||44;mSieges=[];genererPlan();";
  if (contenu.indexOf(ancien) === -1) return false;
  contenu = contenu.replace(ancien, nouveau);
  return true;
});
...
if (ok === total) {
  ...
  console.log('SUCCES : San a maintenant le meme systeme que Bamako.');
} else {
